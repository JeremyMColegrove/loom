...
fn watch_path_glob_allows_matching_paths_only() {
    ...
    let canonical_script_dir =
        std::fs::canonicalize(script_dir.path()).expect("script dir should canonicalize");
...
fn write_and_import_path_globs_are_enforced() {
    ...
    std::fs::create_dir_all(import_allowed.parent().expect("import allowed parent"))
        .expect("mkdir");
    std::fs::create_dir_all(import_denied.parent().expect("import denied parent")).expect("mkdir");
