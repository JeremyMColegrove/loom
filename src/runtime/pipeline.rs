...
impl Runtime {
    ...
    pub(crate) fn run_flow_operations<'a>(&'a mut self, flow: &'a PipeFlow, mut current_value: Value, atomic_started_here_initial: bool) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Value, String>> + 'a>> {
        Box::pin(async move {
    ...
    pub(crate) fn eval_destination<'a>(&'a mut self, op: &'a PipeOp, dest: &'a Destination, pipe_val: Value) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Value, String>> + 'a>> {
        Box::pin(async move {
        match dest {
    ...
    pub(crate) fn handle_on_fail<'a>(&'a mut self, on_fail: &'a OnFail, error: &'a str) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Value, String>> + 'a>> {
        Box::pin(async move {
        self.env.push_scope();
    ...
    pub(crate) fn execute_branch<'a>(&'a mut self, branch: &'a Branch) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Value, String>> + 'a>> {
        Box::pin(async move {
    ...
    pub(crate) fn execute_branch_with_input<'a>(&'a mut self, branch: &'a Branch, input: Value) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Value, String>> + 'a>> {
        Box::pin(async move {
            let mut last_value = input.clone();
            for item in &branch.items {
                let BranchItem::Flow(flow) = item else { continue; };
                self.env.push_scope();
    ...
    pub(crate) fn run_branch_flow_with_input<'a>(&'a mut self, flow: &'a PipeFlow, input: Value) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Value, String>> + 'a>> {
        Box::pin(async move {
    ...
    pub(crate) fn eval_branch_source_with_input<'a>(&'a mut self, source: &'a Source, input: Value) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Value, String>> + 'a>> {
        Box::pin(async move {
            match source {
    ...
    pub(crate) fn eval_source<'a>(&'a mut self, source: &'a Source) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Value, String>> + 'a>> {
        Box::pin(async move {
            match source {
